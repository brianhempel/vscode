...
def char_span_els(string, index, is_special, highlight=None, model=None, scroll_to=False) -> List[str]:
    ...
    if pat_html or repetition_html:
        return [pat_html, f'<span class="char-span-container" {mouse_listener}{scroll_attr}><span class="{" ".join(classes)}">', html.escape(string) if string in HTML_ESCAPE_CHARS else string, '</span></span>', repetition_html]
    else:
        return [f'<span class="char-span-container" {mouse_listener}{scroll_attr}><span class="{" ".join(classes)}">', html.escape(string) if string in HTML_ESCAPE_CHARS else string, '</span></span>']
