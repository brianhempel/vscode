...
from visualizer_utils import (modifier_key_label, replace_dollars_in_py_exp, Unlink, Relink, LineChanged, handle_line_changed, truncate_repr, ICONS,
                              ...
                              dollar_expr_parses, is_nested, Phantom, label_readings,
...
def _real_char_after_internal_idx(string_value: str, idx: int) -> str:
    ...
    Extraction walks the whole string, so this is for one-off lookups only --
    never call it once per match of a possibly-dense pattern."""
...
def visualize_els(value, model, get_visualizer, eval_in_scope, max_width=None, max_height=None, small=False, var_and_exp=None, every_row_exps=None) -> List[str]:
    ...
    if model is not None:
        ...
        # What a segment stops at, keyed the way the dropdown ids are, so the
        # pattern dropdown can offer [^N]. Reading the character through the
        # sentinel-skipping helper rather than the one position past the end
        # matters: a segment ending at a line break stops on the `$` sentinel,
        # which would otherwise read as nothing to stop at.
        # Computed ONLY for the open fuzzy-pattern dropdown -- its renderer is
        # the sole reader, and it reads its own key. Computing one per match
        # of a dense pattern cost a string walk per match (a minute of them).
        model['_segmentNextChars'] = {}
        od = model.get('openDropdown')
        od_id = od.get('id') if od else None
        if od_id and od_id.startswith('fuzzy-pattern-'):
            od_key = od_id[len('fuzzy-pattern-'):]
            for h in highlights:
                if h[5] is not None and f'{h[6]}-{h[5]}' == od_key:
                    model['_segmentNextChars'][od_key] = _real_char_after_internal_idx(value, h[1])
                    break
...
def update(event, var_and_exp, model: dict, value: str, get_visualizer=None, eval_in_scope=None) -> Tuple[dict, List[Any]]:
    ...
    if model.get('linked_action') and not isinstance(msg, (ActionButtonClick, ActionButtonDwell, FetchClick, Unlink, Relink, LineChanged)):
        ...
        ctx = _get_search_context(model, var_and_exp,
                                  source_expr=model['linked_source_expr'],
                                  eval_in_scope=eval_in_scope)
        # The index tool reaches into the string by position, and only Slice,
        # Replace and Delete have a form for that (the grammar's is_index /
        # is_slice templates): any other action generates nothing and the line
        # went stale. So the link moves onto Slice. One way, like the map rule
        # above: a search that stops being positional leaves it there.
        if (_ctx_is_index_or_slice(ctx)
                and model['linked_action'] not in _INDEX_ACTIONS):
            study_note(action='link.set-action', to='find_or_map', why='index')
            model['linked_action'] = 'find_or_map'
        if ctx:
...
# The actions with a form for a positional (index or slice) search.
_INDEX_ACTIONS = frozenset({'find_or_map', 'replace', 'delete'})
