...
User events trigger a re-run to get back to the appropriate visualizer to run the event through its Elm-like update and visualize functions. To speed this up, Python is preloaded at two checkpoints to avoid paying startup costs. Checkpoint 1 is after the python_runner.py is started up; checkpoint 2 is after just the `import`s on the open file (to avoid re-paying import cost). Checkpoint 2 is used when the open file's leading `import`s are unchanged -- which is nearly always, since editing the body doesn't touch them -- and checkpoint 1 otherwise.
